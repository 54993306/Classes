...
CLuckyWheelLayer::CLuckyWheelLayer(void):
	...
	m_oldNoticeLabNum(0),
	m_currNoticeLab(0),
	m_alreadyNoticeLabNum(0),
	m_isUpdateNotice(false)
{
}
CLuckyWheelLayer::~CLuckyWheelLayer(void)
{
	vector<CPrize >::iterator _iterPrize = m_getPrizeList.begin();
	for (;_iterPrize!= m_getPrizeList.end();)
		_iterPrize = m_getPrizeList.erase(_iterPrize);
	m_getPrizeList.clear();

	_iterPrize = m_prizeList.begin();
	for (;_iterPrize!= m_prizeList.end();)
		_iterPrize = m_prizeList.erase(_iterPrize);
	m_prizeList.clear();

	vector<CNotice>::iterator _iterNotice = m_noticeList.begin();
	for (;_iterNotice!= m_noticeList.end();)
		_iterNotice = m_noticeList.erase(_iterNotice);
	m_noticeList.clear();

	vector<CLayout* >::iterator _iterLab = m_noticeLab.begin();
	for (;_iterLab!=m_noticeLab.end();)
		_iterLab = m_noticeLab.erase(_iterLab);
	m_noticeLab.clear();
...
bool CLuckyWheelLayer::init()
{
	if(BaseLayer::init())
	{
		...
		m_pLayout = (CLayout* )m_ui->findWidgetById("NoticeLabel");
		m_pLayout->retain();
...
void CLuckyWheelLayer::ProcessMsg(int type, google::protobuf::Message* msg)
{
	...
	if(type == LuckyWheelMsg)//转盘信息
	{
		...
		//m_oldNoticeLabNum = m_noticeList.size();
		//CCLOG("m_oldNoticeLabNum_%d",m_oldNoticeLabNum);
		//接收上一次的列表的最后一个元素
		//if (m_noticeList.size() != 0)
		//	m_judgePrize = m_noticeList.at(m_noticeList.size()-1);
		...
		CCLOG("nowNoticeLabNum_%d",m_noticeList.size());
		...
		case 1://成功
			{
				if (m_lotteryType == onceLottery)
				{
					...
					showOnceRotate();
					updateLotteryItem();//设置免费或者抽奖需要勾玉数
...
void CLuckyWheelLayer::callBackForShowGainPrize()
{
	...
	else if (m_lotteryType == tenLottery)
	{
		if (m_recordTime == 10)
		{
			...
			popItem->popPrizeResTwoLine(&prizes);
	...
	for (int i = 0 ; i < m_prizeList.size();i++)
	{
		...
		if(iPrize.thumb >= 0)
		{
			...
			if (!spItem)
			{
				...
				CCLOG("error load image %d",iPrize.thumb);
			}
...
void CLuckyWheelLayer::updateNoticeLabelShow()
{
	...
	CLayout *layout = m_pLayout;

	//if (m_oldNoticeLabNum < m_noticeList.size())
	//{
		for (int i =0; i< m_noticeList.size(); i++)
		{
			...
			CCString* strNum = CCString::createWithFormat("%d",data->num);//i
			
			CLabel* name = (CLabel* )cloneLay->findWidgetById("Name");
			CLabel* prize = (CLabel* )cloneLay->findWidgetById("Prize");
			CLabel* num = (CLabel* )cloneLay->findWidgetById("Num");
			
			CLabel* gongxi = (CLabel* )cloneLay->findWidgetById("Label_1");
			CLabel* getprize = (CLabel* )cloneLay->findWidgetById("Label_2");
			CLabel* _x = (CLabel*)cloneLay->findWidgetById("Label_3");
			
			name->setString(strName->getCString());
			prize->setString(strPrize->getCString());
			num->setString(strNum->getCString());

			name->setPositionX(gongxi->getPositionX() + gongxi->getContentSize().width + 5);
			getprize->setPositionX(name->getPositionX() + name->getContentSize().width + 5);
			prize->setPositionX(getprize->getPositionX() + getprize->getContentSize().width + 5);
			_x->setPositionX(prize->getPositionX() + prize->getContentSize().width + 5);
			num->setPositionX(_x->getPositionX() + _x->getContentSize().width + 5);

			ccColor3B ccc = ccc3(0,0,0);
			for (int i = 0;i<m_prizeList.size();i++)
			{
				if (data->id == m_prizeList.at(i).id)
				{
					switch (m_prizeList.at(i).color)
					{
					case 1:
	...
	////覆盖掉之前的数据
	//else if (m_oldNoticeLabNum >= m_noticeList.size())
	//{
	//	bool isStartUpdateNotice = false;
	//	for (int i =0; i< m_noticeList.size(); i++)
	//	{
	//		if (m_noticeList.size()-1 == i  &&isStartUpdateNotice == false)
	//		{
	//			i=0;
	//			isStartUpdateNotice = true;
	//		}
	//		if (i < m_noticeList.size()-1 &&isStartUpdateNotice == false)
	//		{
	//			//判断
	//			int judgeStr =  strcmp(m_judgePrize.player.c_str(),m_noticeList[i].player.c_str());
	//			CNotice noticeLab =  m_noticeList[i];
	//			if (judgeStr == 0&&  m_judgePrize.id == noticeLab.id&&
	//				m_judgePrize.num == noticeLab.num)
	//			{
	//				isStartUpdateNotice = true;
	//				i++;
	//			}
	//		}
	//		if(isStartUpdateNotice)
	//		{
	//			CCLOG("judeg_%d",i);
	//			CLayout* cloneLay = UICloneMgr::cloneLayout(layout);
	//			m_ui->addChild(cloneLay,-9);
	...
	CCLOG("m_noticeLab.size()_%d",m_noticeLab.size());
