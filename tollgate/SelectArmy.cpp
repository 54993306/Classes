...
{ 
	...
	pClose->setOnClickListener(this,ccw_click_selector(CSelectArmy::onClose));
	this->addChild(pClose, 999);


	CButton *combat= (CButton *)m_ui->findWidgetById("combat");
	combat->setOnClickListener(this,ccw_click_selector(CSelectArmy::onCombat));
	...
	m_armyView->setDataSourceAdapter(this,ccw_datasource_adapter_selector(CSelectArmy::armyDataSource));
	...
	m_gridView->setDataSourceAdapter(this,ccw_datasource_adapter_selector(CSelectArmy::gridviewDataSource));
	...
	{
		friendBat->setOnCheckListener(this, ccw_check_selector(CSelectArmy::onFriendBat));
	}
	GetTcpNet->registerMsgHandler(HeroListMsg,this,CMsgHandler_selector(CSelectArmy::processMessage));
	GetTcpNet->registerMsgHandler(UnionRes,this,CMsgHandler_selector(CSelectArmy::processMessage));

	NOTIFICATION->addObserver(this, callfuncO_selector(CSelectArmy::willToBatthle), RECEIVE_BATTLE_INFO, nullptr);
...
CCObject* CSelectArmy::armyDataSource(CCObject* pConvertCell, unsigned int uIdx)
{
...
void CSelectArmy::onClose(CCObject* pSender)
{
...
void CSelectArmy::onClearHero(CCObject* pSender)
{
...
void CSelectArmy::processNetMsg(int type, google::protobuf::Message *msg)
{
...
void CSelectArmy::setUnionList(vector<CUnion> unionList)
{
			...
			case 1:
				{
					if (hero.thumb>0)
					{
						...
						if (!head)
						{
							...
							CCLOG("[ ERROR ] CSelectArmy::addHeroCell Lost Image = %d",hero.thumb);
						}
						head->setScale(0.92f);
...
void CSelectArmy::onCombat(CCObject* pSender)
{
	CCLOG("CSelectArmy::onCombat");
...
void CSelectArmy::onMyBat(CCObject *pSender, bool bChecked)
{
...
void CSelectArmy::onFriendBat(CCObject *pSender, bool bChecked)
{
...
CCObject* CSelectArmy::gridviewDataSource(CCObject* pConvertCell, unsigned int uIdx)
{ 
...
void CSelectArmy::onSwitchBtn(CCObject *pSender, bool bChecked)
{
...
void CSelectArmy::updateHeroList()
{
	if (m_selectType==1)
	{
...
void CSelectArmy::addGridCell(unsigned int uIdx, CGridViewCell* pCell)
{
	...
	for (int i = 1; i <=22; i++)
	{
		...
		else if (i==3)
		{
			...
			{
				head = CCSprite::create("headImg/101.png");
				CCLOG("[ ERROR ] CSelectArmy::addGridCell Lost Image = %d",hero->thumb);
			}	
	...
	if (hero->armor1.id>0&&hero->armorNum>0)
	{
		...
		if (!item)
		{
			...
			CCLOG("[ ERROR ] CSelectArmy::addGridCell Lost Image = %d",hero->armor1.iconId);
		}
	...
	if (hero->armor2.id>0&&hero->armorNum>1)
	{
		...
		{
			item = CCSprite::create("prop/32003.png");
			CCLOG("[ ERROR ] CSelectArmy::addGridCell Lost Image = %d",hero->armor2.iconId);
		}
		item->setPosition(ccp(img->getContentSize().width/2,img->getContentSize().height/2));
		img->addChild(item);
...
void CSelectArmy::addHeroSelectCell(unsigned int uIdx, CGridViewCell* pCell)
{
	CHero *hero = m_currHeroList->at(uIdx);
	CLayout *lay = UICloneMgr::cloneLayout(m_cell);
...
void CSelectArmy::showSelectRadioImg(int selIndex)
{
	
}

void CSelectArmy::onSelectHero(CCObject *pSender)
{
...
void CSelectArmy::processMessage(int type, google::protobuf::Message *msg)
{
	if(!isVisible())
	{
		this->setVisible(true);
...
void CSelectArmy::updateCurrHeroList()
{
	CRadioBtnGroup *radioGroup = (CRadioBtnGroup *)m_ui->getChildByTag(10);
	for (int i = 0; i < 4; i++)
	{
...
void CSelectArmy::setStagId(int stageId, int questId/*=0*/)
{
	m_stageId = stageId;
	m_questId = questId;
}

void CSelectArmy::updateCost()
{
...
bool CSelectArmy::findIsSelect(CHero * hero)
{
	bool isfind =false;
	for (int i = 0; i < m_union.heroList.size(); i++)
	{
...
void CSelectArmy::showActionForArmy(float dt)
{
	m_pActiveLay->runAction(CCSequence::createWithTwoActions(CCMoveBy::create(0.25f, ccp(0, 150)), CCCallFunc::create(this, callfunc_selector(CSelectArmy::showActionForArmyCallBack))));
	//链条1.链条2
	{
...
void CSelectArmy::showActionForArmyReverse(float dt)
{
...
void CSelectArmy::showActionForArmyCallBack()
{
	//是否有队长
	if(checkShowCaptain())
	{
...
bool CSelectArmy::showAddCaptain()
{
	...
	//太极开，数字飞出，木板提上来
	{
		...
		//木板
		{
			{
				CCNode* pLink = (CCNode*)m_ui->findWidgetById("link_1");
				pLink->runAction(CCMoveBy::create(0.2f, ccp(0, 107)));
			}
			{
				CCNode* pLink = (CCNode*)m_ui->findWidgetById("link_2");
				pLink->runAction(CCMoveBy::create(0.2f, ccp(0, 107)));
	...
	this->runAction(CCSequence::createWithTwoActions(CCDelayTime::create(0.4f), CCCallFunc::create(this, callfunc_selector(CSelectArmy::showCaptainActionCallBack))));

	return true;
}

bool CSelectArmy::showRemoveCaptain()
{
	if(m_bCaptainShowAction)
	{
		return false;
...
void CSelectArmy::showCaptainActionCallBack()
{
	m_bCaptainShowAction = false;
}

bool CSelectArmy::checkShowCaptain()
{
	const CHero &hero = m_union.heroList.at(0);
	if (hero.id!=0&&hero.skill5.skillId>0)
	{
