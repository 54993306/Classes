...
void CMainScene::onCreate()
{
	...
	m_cityUi->setTouchPriority(-1);
	this->addChild(m_cityUi);
// 	LayerManager::instance()->push(m_cityUi);
	
	CTopLayer *topLay = CTopLayer::create();
	topLay->setTouchPriority(-1);
	this->addChild(topLay,100,2000);
...
{	
	...
	for (int i=1; i<=17; ++i)
	{
		...
		if (btn)
		{
			btn->setOnClickListener(this,ccw_click_selector(CMainScene::onClickButton));
	...
	//绑定场景隐藏和显示的消息
	NOTIFICATION->addObserver(this, callfuncO_selector(CMainScene::show), SHOW_MAIN_SCENE, nullptr);
	NOTIFICATION->addObserver(this, callfuncO_selector(CMainScene::hide), HIDE_MAIN_SCENE, nullptr);

	PlayBackgroundMusic(BGM_MainCity,true);

	//不再需要走路的英雄——又需要了
	CSceneManager::sharedSceneManager()->addMsgObserver(GARRSION_HERO,this,GameMsghandler_selector(CMainScene::updateGarrsionHero));
	CSceneManager::sharedSceneManager()->addMsgObserver(UPDATE_BUILD,this,GameMsghandler_selector(CMainScene::updateCityBuild));
	CSceneManager::sharedSceneManager()->addMsgObserver(UPDATE_HERO,this,GameMsghandler_selector(CMainScene::updateMobileShop));

	GetTcpNet->registerMsgHandler(ActListMsg, this, CMsgHandler_selector(CMainScene::recvWorldBossNotice));
	...
	m_armature->getAnimation()->setMovementEventCallFunc(this,movementEvent_selector(CMainScene::moveEvent));
	m_armature->getAnimation()->setFrameEventCallFunc(this,frameEvent_selector(CMainScene::openFrameEvent));
	...
	//user->setNewStep(1);
	this->scheduleOnce(schedule_selector(CMainScene::runGuideStep),0.4f);
#else
	this->scheduleOnce(schedule_selector(CMainScene::runGuideStep),0.4f);
#endif
	...
	if (btn != NULL)
	{
		btn->runAction(CCSequence::create(CCDelayTime::create(0.3f),
			CCCallFuncN::create(this, callfuncN_selector(CMainScene::onTimeWaitCityAction)),
			NULL));
		DataCenter::sharedData()->setCityActionType(CA_None);

	}

	CSceneManager::sharedSceneManager()->addMsgObserver(UPDATE_FUNCTIONOPEN,this,GameMsghandler_selector(CMainScene::updateFuctionOpen));
	CSceneManager::sharedSceneManager()->addMsgObserver(UPDATE_GAMETIP,this,GameMsghandler_selector(CMainScene::updateGameTip));
	showNoticeTip(CTaskControl::getInstance()->getGameTips());

// 	CCSprite *fog = CCSprite::create("mainCity/Build/fog3.png");
// 	fog->setPosition(ccp(-500,VCENTER.y));
// 	fog->runAction(CCRepeatForever::create(CCSequence::createWithTwoActions(CCMoveBy::create(52.0f,ccp(3500,0)),CCCallFuncN::create(this,callfuncN_selector(CMainScene::resetPos)))));
// 	m_frontLay->addChild(fog,-1);
}

void CMainScene::resetPos(CCNode* node)
{
	node->setPosition(ccp(-500,VCENTER.y));
}

void CMainScene::removeMobieShop(float dt)
{
	if (m_armature)
	{
...
void CMainScene::moveEvent(CCArmature *armature, MovementEventType movementType, const char *movementID)
{
...
void CMainScene::openFrameEvent(CCBone *bone, const char *name, int orginIndex, int currendtIndex)
{

}

void CMainScene::runGuideStep(float dt)
{
	UserData *user = DataCenter::sharedData()->getUser()->getUserData();
//	user->setNewStep(1);
...
void CMainScene::updateGarrsionHero(const TMessage& tMsg)
{
...
void CMainScene::onClickButton(CCObject* pSender)
{
...
// 	{
// 		CCString *str = CCString::createWithFormat(GETLANGSTR(202));
...
void CMainScene::onEnterTransitionDidFinish()
{
	m_isLoadFinish = true;
...
void CMainScene::blinkShow(CCNode *node)
{
	node->setVisible(true);
}

void CMainScene::showCityInfo(CityData& cityData)
{
...
// 			this->scheduleOnce(schedule_selector(CMainScene::moveHero),delayTime);
...
void CMainScene::moveHero(float delta)
{
	CCPoint heroPos = m_hero[0]->getPosition();
...
void CMainScene::walkStop(CCNode *node)
{
	...
	CCCallFuncN *call = CCCallFuncN::create(this, callfuncN_selector(CMainScene::repeatMove));
	node->runAction(CCSequence::create(delay,call,nullptr));
}

void CMainScene::repeatMove(CCNode *node)
{
	int indexPos = random_range(0,2);
...
bool CMainScene::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
	if(m_bTouchLock)
	{
		return false;
...
void CMainScene::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent)
{
...
void CMainScene::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent)
{
...
void CMainScene::ccTouchCancelled( CCTouch *pTouch, CCEvent *pEvent )
{
	m_bTouchLock = false;
}

void CMainScene::onGetButton(CCObject* pSender)
{
	int mul = 1;
...
// 		CCMoveBy *moveBy = CCMoveBy::create(0.5f,subPos);
		...
		CCCallFuncN *call = CCCallFuncN::create(this,callfuncN_selector(CMainScene::removeCoin));
...
void CMainScene::removeCoin(CCNode * node)
{
	node->removeFromParent();
}

void CMainScene::runShake(CCNode * lay)
{
...
void CMainScene::updateCityCap(CCity &city, CLayout * lay, CButton * btn)
{
	if (city.capFull)
	{
		lay->setVisible(true);
		btn->setOnClickListener(this, ccw_click_selector(CMainScene::onGetButton));
		runShake(lay);
	}
	else
	{
...
void CMainScene::runCloudAction()
{
	//云运动
// 	CCSprite *cloud  =  (CCSprite *)(m_fogLay->findWidgetById("cloud"));
	...
	CCSequence *sequence = CCSequence::create(move,CCCallFuncN::create(this,callfuncN_selector(CMainScene::relocation)),moveb,nullptr);
...
// 	CCSprite *cloud1  =  (CCSprite *)(m_fogLay->findWidgetById("cloud1"));
	...
	CCSequence *sequence1 = CCSequence::createWithTwoActions(move1,CCCallFuncN::create(this,callfuncN_selector(CMainScene::relocation)));
	m_fogLay1->runAction(CCRepeatForever::create(sequence1));
}

void CMainScene::relocation(CCNode* pSender)
{
...
void CMainScene::runFogAction()
{
	...
	CCSequence *sequence = CCSequence::create(move,CCCallFuncN::create(this,callfuncN_selector(CMainScene::relocationFog)),moveb,nullptr);
	...
	CCSequence *sequence1 = CCSequence::createWithTwoActions(move1,CCCallFuncN::create(this,callfuncN_selector(CMainScene::relocationFog)));
	cloud1->runAction(CCRepeatForever::create(sequence1));
}

void CMainScene::relocationFog(CCNode* pSender)
{
	CLayout *cloud = (CLayout*)pSender;
...
void CMainScene::cityHeroMovePoint()
{
	//武将移动的路径点
...
void CMainScene::addMoveHero(CCity * cb, int i)
{
	//不需要人物出来走动
...
void CMainScene::updateCityGarrsion(CHero *hero, int cityId)
{
...
void CMainScene::updateCityBuild(const TMessage& tMsg)
{
	if (tMsg.lParam)
	{
...
void CMainScene::updateMobileShop(const TMessage& tMsg)
{
...
void CMainScene::updateBuildTexture(CCity * city)
{
...
void CMainScene::createBuildEffect()
{
...
void CMainScene::onTimeWaitCityAction(CCNode* pSender){
	CButton *btn = (CButton*)pSender;
	if (btn != NULL)
	{
...
void CMainScene::moveFrontByPos(CCPoint pos)
{
...
void CMainScene::updateFuctionOpen(const TMessage& tMsg)
{
	int tag = tMsg.nMsg;
	if (tag<20&&tag>0)
	{
...
void CMainScene::addCulAnim(CLayout * effLay)
{
...
void CMainScene::addStrengthenAnim(CCPoint firePos, CLayout * effLay)
{
...
void CMainScene::addComposeAnim(CCPoint lightPos1, CCPoint lightPos2, CLayout * effLay)
{
...
void CMainScene::show(CCObject* pObj)
{
	m_iIndexForShow++;
	...
	CCLOG("CMainScene::show_%d", m_iIndexForShow);
}

void CMainScene::hide(CCObject* pObj)
{
	m_iIndexForShow--;
	if(m_iIndexForShow<=0)
	{
	...
	CCLOG("CMainScene::hide_%d", m_iIndexForShow);
}

void CMainScene::updateGameTip(const TMessage& tMsg)
{
	CGameTips *ct = CTaskControl::getInstance()->getGameTips();
	showNoticeTip(ct);
}

void CMainScene::showNoticeTip(CGameTips * ct)
{
	CCNode *ziti = (CCNode*)(m_frontLay->findWidgetById("ziti"));
...
void CMainScene::hideTips(int tag)
{
	CCNode *ziti = (CCNode*)(m_frontLay->findWidgetById("ziti"));
...
void CMainScene::recvWorldBossNotice( int type, google::protobuf::Message *msg )
{
	CCNode *ziti = (CCNode*)(m_frontLay->findWidgetById("ziti"));
	CCSprite *red = (CCSprite*)ziti->getChildByTag(1603);
	red->setVisible(true);
